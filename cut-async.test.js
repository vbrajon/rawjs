...
const scenarios = [
  ...
  },
  {
    ...
    fn: async () => {
      ...
      await Function.wait(loop.stop, 280) // immediate + runs every 100ms + stops after 250ms
      if (n !== 3) throw new Error(`Function.every should have been called 3 times, n = ${n}`)
    },
  },
  {
  ...
  },
  {
    ...
    fn: async () => {
      ...
      const map = await Promise.map([1, 2], inc)
