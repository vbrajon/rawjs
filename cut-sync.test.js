...
export default [
  () => Object.map(obj, v => v * 2 || v), { name: 'John Doe', age: 58 },
  () => Object.find(obj, v => v > 10), 29,
  () => Object.filter(obj, Number), { age: 29 },
  () => Object.findIndex(obj, 29), 'age',
  () => Object.reduce(obj, (acc, v, k) => ((acc[v] = k), acc), {}), { 'John Doe': 'name', 29: 'age' },
  ...
  () => Object.equal(sorting, sortingClone), true,
  ...
  () => Array.sort(arr, 'age'), Array.sort(arr, function() {
  ...
  () => Array.unique(Array.map(arr, 'age')), [29, 22, 71],
  ...
  // () => sortingShuffled.slice().sort(), sortingNative,
  // () => Array.sort(sortingShuffled), sorting,
  ...
  () => String.format('{user.name} is <strong>{{user.age}}</strong>', { user: obj }), 'John Doe is <strong>{29}</strong>',
  () => String.format('{.length} users starting with {0.name} & {1.name}', arr), '4 users starting with John Doe & Jane Doe',
